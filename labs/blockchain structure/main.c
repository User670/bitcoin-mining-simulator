...
// This is to test dealing with a blockchain data structure.
...
void debug_print_block(BitcoinHeader block){
    printf("====Debug print block start====\n");
    printf("version:       %d\n", block.version);
    ...
    printf("timestamp:     %d\n", block.timestamp);
    printf("difficulty:    ");
      debug_print_hex(&(block.difficulty),4);
      printf("\n");
    printf("nonce:         %d\n", block.nonce);
    printf("====Debug print block end====\n");
...
int main(){
    ...
    BitcoinHeader blocks[num_of_blocks];
    
    for(int i=0; i<num_of_blocks; i++){
        blocks[i].version=2; // the actual Genesis block was on version 1
        if(i==0){
            // the "previous block hash" for the Genesis block was
            // 32 bytes of zeros
            memset(&(blocks[i].previous_block_hash),0,32);
        }else{
            // calculate a hash from prev block
            dsha(&(blocks[i-1]), sizeof(BitcoinHeader), &(blocks[i].previous_block_hash));
        }
        // do random for current block's merkle root
        get_random_hash(&(blocks[i].merkle_root));
        blocks[i].timestamp=time(NULL);
        blocks[i].difficulty=difficulty;
        blocks[i].nonce=0;
        
        // normally here I should mine the block to find a valid nonce
        // for this experiment pretend any nonce is valid
