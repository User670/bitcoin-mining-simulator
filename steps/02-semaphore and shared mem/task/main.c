...
typedef struct{
    BitcoinHeader block;
    char* target;
    // TODO: Add other shared data that you need.
}SharedData;
...
void process_miner(int id, SharedData* sd){
    ...
    while(1){
        ...
        // only do this when you are sure parent has sent a new block header
        working_block=sd->block;
        
        // Here's the brute force loop for your reference, but you will likely
        // add more logic to make it work
